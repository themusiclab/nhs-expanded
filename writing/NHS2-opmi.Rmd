...
In Figure 4, we examine this feature within the three deeply-sampled language families, and across the whole corpus. The overall effect of behavioral context is evident: dance songs, as a class, are higher in spectral entropy than lullabies (Figure 4A-B). Additionally, spectral entropy values also differ between dance songs and lullabies *within each language family* (Figure 4C). These descriptive visualizations help reveal how acoustics vary between cultures and behavioral contexts, highlighting the substantial variability within each language family, while demonstrating the general trend that spectral entropy can coarsely differentiate between dance songs and lullabies. This association may track with differences in the purpose of dance music and lullabies, especially with regards to up- or down-regulating emotional arousal. However, it remains unknown the degree to which this association is grounded more in physiological aspects of our music psychology or in some culturally reinforced convention that is likely to turn out otherwise if we were to replay the tape of life.

Beyond this specific example of spectral entropy, we also ask the more general question of what the relative influences of behavioral context and phylogeny have on the acoustic features of songs. We used Bayesian phylogenetic modeling to estimate the group-level effects of songs' behavioral context, language, as well as linguistic ancestry (henceforth *phylogeny*) and geographical proximity on its acoustic features. The model was estimated in `brms` [@Burkner2020], modeling random group effects using Gaussian processes and computing intra-class correlation (ICC) as the proportion of variance explained by each variable. The acoustic features used here are the same `MIRtoolbox` features described in the previous section. Phylogenetic distance between languages was modeled assuming Brownian diffusion of traits on a single Maximum Clade Credibility language tree describing language geneology across the globe [@Bouckaert2022]. Geospatial proximity was calculated using `geosphere` [@Burkner2022] on geospatial data present in the *Glottolog* [@Hammarstrom2023] for each language, assuming covariance is exponentially proportional to geographic distance. 
